document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const res = await window.apiFetch(`/adopciones/mis?idAnimalLover=${myIdStr}`);
        let publicaciones = [];
        if (res && res.ok) {
        ...
        if (!publicaciones || publicaciones.length === 0) {
            ...
            if (adopcionesList) adopcionesList.style.display = 'none';
            return;
        }

        if (emptyState) emptyState.style.display = 'none';
        if (adopcionesList) {
            adopcionesList.style.display = 'block';
            adopcionesList.innerHTML = '';

            for (const item of publicaciones) {
                const data = item.adopcion;
                ...
                const badgeColor = estado === 'Adoptado' ? '#22c55e' : '#f59e0b';
                const badgeLabel = estado === 'Adoptado' ? '✓ Adoptado' : '● Activo';
                ...
                mainInfo.innerHTML = `
                    ...
                    <p style="margin:4px 0; color:#555; font-size:0.9rem;">Vacunas: <span>${data.vacunas}</span></p>
                `;
                ...
                btnEdit.onclick = () => {
                    window.location.href = `pubAdopcion.html?edit=${data.idAnimal}`;
                };

                const btnAdoptado = document.createElement('button');
                btnAdoptado.className = 'btn';
                btnAdoptado.innerHTML = '<i class="fas fa-check"></i> Adoptado';
                btnAdoptado.style.cssText = 'flex:1; background-color:#22c55e; color:#fff; border:none; border-radius:12px; padding:10px; font-weight:600; cursor:pointer;';

                if (estado === 'Adoptado') {
                    btnAdoptado.disabled = true;
                    btnAdoptado.style.opacity = '0.6';
                    btnAdoptado.style.cursor = 'not-allowed';
                }

                btnAdoptado.onclick = async () => {
                    if (!confirm('¿Marcar este animal como adoptado?')) {
                        return;
                    }
                    try {
                        const resAdoptado = await window.apiFetch(`/adopciones/adoptado?id_animal=${data.idAnimal}`, {
                            method: 'POST'
                        });
                        if (resAdoptado && resAdoptado.ok) {
                            btnAdoptado.disabled = true;
                            btnAdoptado.style.opacity = '0.6';
                            btnAdoptado.style.cursor = 'not-allowed';
                            mainInfo.querySelector('span').style.background = '#22c55e';
                            mainInfo.querySelector('span').textContent = '✓ Adoptado';
                        } else if (resAdoptado) {
                            const error = await resAdoptado.json();
                ...
                btnDelete.onclick = async () => {
                    ...
                    try {
                        const resDel = await window.apiFetch(`/adopciones?id_animal=${data.idAnimal}`, {
                            method: 'DELETE'
                        });
                        if (resDel && resDel.ok) {
                            article.remove();
                            if (adopcionesList.children.length === 0 && emptyState) {
                                emptyState.style.display = 'block';
                                adopcionesList.style.display = 'none';
                            }
                        } else if (resDel) {
                ...
                actions.appendChild(btnAdoptado);
                ...
                adopcionesList.appendChild(article);
            }
        }
    } catch (e) {
        console.error('Error obteniendo adopciones:', e);
        if (emptyState) {
            emptyState.style.display = 'block';
