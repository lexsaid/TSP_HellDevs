...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    let detalle;
    try {
        const res = await window.apiFetch(`/adopciones/detalle?id_animal=${idAnimal}`);
        if (!res || !res.ok) {
    ...
    const dueno = detalle.publicador;

    document.getElementById('adopcionNombre').textContent = data.nombre || 'Sin nombre';
    document.getElementById('adopcionDetalles').innerHTML = `
    ...
    `;
    document.getElementById('adopcionUbicacion').textContent = 'Ubicacion: ' + (data.direccion || '--');
    document.getElementById('adopcionVacunas').textContent = `Vacunas: ${data.vacunas || '--'}`;
    document.getElementById('adopcionEdad').textContent = `Edad: ${data.edad ?? '--'} meses`;
    document.getElementById('adopcionExtra').textContent = `Condicion: ${data.discapacidad || 'Ninguna'}`;
    ...
    btnContactar.addEventListener('click', () => {
        const idTrabajo = -(2000000 + idAnimal);
